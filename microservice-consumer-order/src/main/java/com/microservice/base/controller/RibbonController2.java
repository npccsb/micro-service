...
public class RibbonController2 {
	private static final Logger LOG = LoggerFactory.getLogger(RibbonController2.class);
	...
	public User getUser(@PathVariable("id") Long id) {
		User o = restTemplate.getForObject("http://microservice-provider-user2/user/" + id, User.class);
		return o;
	}

	@PostMapping("/user/post-add")
	public User addUser(@RequestBody User user) {
		User o = restTemplate.postForObject("http://microservice-provider-user2/user/post-add", user, User.class);
		return o;
	}
	...
	public User addUser2(User user) {
		User o = restTemplate.getForObject("http://microservice-provider-user2/user/get-add", User.class, user);
		return o;
	}

	@GetMapping("/role/{id}")
	public Role getRole(@PathVariable("id") Long id) {
		Role o = restTemplate.getForObject("http://microservice-provider-user2/role/" + id, Role.class);
		return o;
	}

	@PostMapping("/role/post-add")
	public Role addRole(@RequestBody Role role) {
		Role o = restTemplate.postForObject("http://microservice-provider-user2/role/post-add", role, Role.class);
		return o;
	}

	@GetMapping("/complex1/{id}")
	public ComplexBean1 testComplexBean1(@PathVariable("id") Long id) {
		ComplexBean1 o = restTemplate.getForObject("http://microservice-provider-user2/complex1/" + id,
				ComplexBean1.class);
		return o;
	}
	...
	public ComplexBean1 postAddComplexBean1(@RequestBody ComplexBean1 bean) {
		ComplexBean1 o = restTemplate.postForObject("http://microservice-provider-user2/complex1/post-add", bean,
				ComplexBean1.class);
		return o;
	}
	...
	public ComplexBean1 getAddTestComplexBean1(ComplexBean1 bean) {
		ComplexBean1 o = restTemplate.getForObject("http://microservice-provider-user2/complex1/get-add",
	...
	public ComplexBean2 getComplexBean2(@PathVariable("id") Long id) {
		ComplexBean2 o = restTemplate.getForObject("http://microservice-provider-user2/complex2/" + id,
				ComplexBean2.class);
		return o;
	}
	...
	public ComplexBean2 postAddComplexBean2(@RequestBody ComplexBean2 bean) {
		ComplexBean2 o = restTemplate.postForObject("http://microservice-provider-user2/complex2/post-add", bean,
				ComplexBean2.class);
		return o;
	}
	...
	public ComplexBean2 getAddTestComplexBean2(ComplexBean2 bean) {
		ComplexBean2 o = restTemplate.getForObject("http://microservice-provider-user2/complex2/get-add",
	...
	/**
	 * 通过断路器证明，底层的这种方法，不会抛出异常，而是返回一个空对象，也就是说不会走断路器
	 */
	@HystrixCommand(fallbackMethod = "complex2Fallback")
	...
	public List<User> testList(@RequestBody List<User> ulist) {
		...
		List<User> o = RestTemplateUtils.getForListObject(
				restTemplate.postForObject("http://microservice-provider-user2/user-list", ulist, User[].class));
	...
	public Map<Long, User> testMap(@RequestBody Map<Long, User> umap) {
		Object o = restTemplate.postForObject("http://microservice-provider-user2/user-map", umap, Map.class);
	...
	public Set<User> testSet(@RequestBody Set<User> uset) {
		...
		//Object o = restTemplate.postForObject("http://microservice-provider-user2/user-set",  uset, Set.class);
		Set<User> o = RestTemplateUtils.getForSetObject(restTemplate.postForObject("http://microservice-provider-user2/user-set",  uset, User[].class));
		return (Set<User>) o;
	}
	...
	public List<User> testListEntity(@RequestBody List<User> ulist) {
		...
		@SuppressWarnings("unchecked")
		ResponseEntity<List<User>> u = (ResponseEntity<List<User>>) restTemplate.postForEntity("http://microservice-provider-user2/user-list", ulist, o.getClass());
		o = u.getBody();
		return o;
	}
	...
	public Map<Long, User> testMapEntity(@RequestBody Map<Long, User> umap) {
		...
		@SuppressWarnings("unchecked")
		ResponseEntity<Map<Long, User>> u = (ResponseEntity<Map<Long, User>>) restTemplate.postForEntity("http://microservice-provider-user2/user-map", umap, o.getClass());
		o = u.getBody();
		return o;
	}
	...
	public Set<User> testSetEntity(@RequestBody Set<User> uset) {
		...
		@SuppressWarnings("unchecked")
		ResponseEntity<Set<User>> u = (ResponseEntity<Set<User>>) restTemplate.postForEntity("http://microservice-provider-user2/user-set", uset, o.getClass());
		o = u.getBody();
		return o;
	}
	...
	public String testHystrix(@PathVariable String arg){
		return restTemplate.getForObject("http://microservice-provider-user2/test-hystrix/"+arg, String.class);
	}
	/**
